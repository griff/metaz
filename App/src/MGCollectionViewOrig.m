...
static NSString * const DRAG_ITEM_TYPE = @"MGCollectionViewOrigDragType";
...
@interface MGCollectionViewOrig (Private)
...
- (void)startDrag: (NSEvent *)event
         withItem: (MGCollectionViewOrigItem *)item;
- (NSImage *)dragImageForIndexes:(NSIndexSet *)indexes
                     dragPoint: (NSPoint *)dragPoint;
- (void)itemClicked: (MGCollectionViewOrigItem *)item
              event: (NSEvent *)event;
- (void)setAllItemsSelected: (BOOL)selected;
- (void)growSelectionToItem: (MGCollectionViewOrigItem *)item;
...
@interface MGCollectionViewOrigItem (Private)
...
- (BOOL)isSelected;
- (MGCollectionViewOrig *)collectionView;
...
@implementation MGCollectionViewOrig
...
- (void)awakeFromNib
{
    NSAssert(!self.target || [self.target conformsToProtocol: @protocol(MGCollectionViewOrigTarget)], @"Bad target");
/*
   [[NSNotificationCenter defaultCenter]
      addObserver:self
         selector:@selector(onKeyWindowChanged:)
...
- (void)selectIndexes:(NSIndexSet *)indexes byExtendingSelection:(BOOL)extend
{
    ...
    for(MGCollectionViewOrigItem* item in [items objectsAtIndexes:indexes])
...
@end // MGCollectionViewOrig


@implementation MGCollectionViewOrig (Private)
          ...
          byExtending: (BOOL)byExtending
{
    ...
    if (index != NSNotFound) {
        ...
        MGCollectionViewOrigItem *item = [items objectAtIndex:index];
        [item setIsSelected:YES];
        [self scrollToItem:item];
    }
...
- (void)removeItemsAtIndexes: (NSIndexSet *)indexes
{
    ...
    for(MGCollectionViewOrigItem *item in to_remove)
...
@end // MGCollectionViewOrig (Private)


@implementation MGCollectionViewOrigItem
...
@end // MGCollectionViewOrigItem

@implementation MGCollectionViewOrigItem (Private)
...
- (MGCollectionViewOrig *)collectionView
{
    ASSERT([[self superview] isKindOfClass:[MGCollectionViewOrig class]]);
    return (MGCollectionViewOrig *)[self superview];
}
...
@end // MGCollectionViewOrigItem (Private)